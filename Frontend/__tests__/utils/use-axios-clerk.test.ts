...
test('attaches interceptor and sets Authorization header when token exists', async () => {
	...
	expect(axiosMock.create).toHaveBeenCalledWith({ baseURL: process.env.EXPO_PUBLIC_API_BASE_URL });
...
describe('Route Builders', () => {
	it('builds static routes across services', () => {
		const staticRoutes = [
			...
			[GO_MESSAGING_ROUTES.DIRECT_CONVERSATION, 'api/v1/messaging/conversations/direct'],
	...
	it('builds parameterized routes across services', () => {
		...
		expect(GO_USER_SERVICE_ROUTES.BY_ID(userId)).toBe(`api/v1/user/id/${userId}`);
