...
const initialFormState = {
  ...
  region: "Canada",
  location: "Montreal",
  level: "COMPETITIVE" as (typeof leagueLevels)[number],
};
...
export function CreateLeagueDialog({ onCreated }: { onCreated: () => void }) {
    ...
    try {
      ...
      if (!response.ok) {
        toast.error("Failed to create league", {
          description: body?.error ?? "Unable to create league.",
        });
        return;
      }

      toast.success("League created", {
        description: `${body?.league?.name ?? form.name} was created successfully.`,
      });
      ...
      toast.error("Failed to create league", {
        description: "Unable to create league right now.",
      });
    } finally {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="border-white/10">
        <DialogHeader>
          ...
          <DialogDescription>
            Create a league, choose who owns it, and define the key details
            needed to organize and manage matches.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit}>
          <div className="px-6">
            <FieldGroup className="gap-5">
              <Field>
                <FieldLabel htmlFor="league-owner">Owner</FieldLabel>
                <Select
                ...
                >
                  <SelectTrigger id="league-owner" className="w-full">
                    <SelectValue
                      placeholder={
              ...
              <Field>
                <FieldLabel htmlFor="league-sport">Sport</FieldLabel>
                <Select
                  value={form.sport}
                  onValueChange={(value) =>
                ...
                >
                  <SelectTrigger id="league-sport" className="w-full">
              ...
              <Field>
                <FieldLabel htmlFor="league-level">Level</FieldLabel>
                <Select
                  value={form.level}
                  onValueChange={(value) =>
                    setForm((current) => ({
                      ...current,
                      level: value as (typeof leagueLevels)[number],
                    }))
                  }
                >
                  <SelectTrigger id="league-level" className="w-full">
              ...
              <Field>
                <FieldLabel htmlFor="league-region">Region</FieldLabel>
                <Input
                  id="league-region"
                  value={form.region}
                  onChange={(event) =>
              ...
              <Field>
                <FieldLabel htmlFor="league-location">Location</FieldLabel>
                <Select
                  value={form.location}
                  onValueChange={(value) =>
                ...
                >
                  <SelectTrigger id="league-location" className="w-full">
