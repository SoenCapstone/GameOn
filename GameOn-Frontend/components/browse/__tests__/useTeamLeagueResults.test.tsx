...
describe('useTeamLeagueResults', () => {
  it('returns teams reversed then leagues', () => {
    useTeamResults.mockReturnValue({
      data: [
        { id: 't1', type: 'team', name: 'Team One', subtitle: 'soccer', logo: 'L1', league: '' },
        { id: 't2', type: 'team', name: 'Team Two', subtitle: 'soccer', logo: 'L2', league: '' },
      ],
      isLoading: false,
      error: null,
    });
    ...
    filterLocalLeagues.mockReturnValue([
      { id: 'l1', type: 'league', name: 'League A', subtitle: 'league', logo: '🏅', league: 'A' },
    ]);
  ...
  it('handles empty team data', () => {
    ...
    const { getByTestId } = render(<TestComp query="x" />);
    const ids = JSON.parse(getByTestId('out').props.children as string) as string[];
